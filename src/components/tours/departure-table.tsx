  ...
  return (
    <div className="w-full bg-[#f5ca91] px-4 py-8 md:px-10 md:py-12">
      <div className="overflow-hidden rounded-2xl border border-black/10 shadow-sm">
        ...
        {/* ── Header ─────────────────────────────────────────────────── */}
        <div className="grid grid-cols-[1fr_2fr_0.6fr_1.2fr_0.7fr_1.4fr_1fr] bg-[#c8a06a] px-6 py-4 text-center text-[15px] font-semibold text-white md:text-[17px]">
          <span>出發日期</span>
          <span>行程名稱</span>
          <span>天數</span>
          <span>航空公司</span>
        ...
        <div className="bg-[#f5ca91]">
          {rows.map((row, i) => (
            <div
              key={i}
              className="grid grid-cols-[1fr_2fr_0.6fr_1.2fr_0.7fr_1.4fr_1fr] items-center border-b border-black/10 px-6 py-5 text-center text-[15px] last:border-b-0 md:py-6 md:text-[16px]"
            >
              <span className="italic text-black/70">{row.date}</span>
